...
require_once '../blossom-php/mwmatching.php';


class HeadToHeadBetterSwissPairing extends Base
{
	...
	public function build() {
		...
		foreach ($matching as $playerIndex => $opponentIndex) {
			...
			if ($playerIndex >= $opponentIndex) continue;
			// Player indices were increased by 1 to make room for the bye at 0, now decrease them again
			if ($playerIndex === 0) $byes []= $players[$opponentIndex - 1];
			else $pairings []= [$players[$playerIndex - 1], $players[$opponentIndex - 1]];
		}
